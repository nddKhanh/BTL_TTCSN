...
- `@Transactional(readOnly = true)` for reads that touch lazy associations/collections (with
  `spring.jpa.open-in-view: false`, the repository call's own transaction closes before the
  mapping code runs — any lazy field accessed afterward throws `LazyInitializationException`)
- Controllers depend **only** on Services — never inject a `Repository` into a `@RestController`
...
- **Inject a `*Repository` (or another domain's repository) into a Controller.** Query/aggregation
  logic — including response-shaping/serialization helpers like `toXxxMap` — belongs in the
  Service layer, not the Controller. If a controller method needs data, add/extend a Service
  method; don't reach past it to the repository "just this once." This has recurred in this repo
  (`ProblemController` pulled in `ProblemRepository`/`ProblemTagRepository`/`ContestRepository`
  directly) — treat any `Repository` field on a class annotated `@RestController` as a review
  blocker.
