...
## DX-Farm Convention: `@Query` Text Block Formatting

Nếu backend dùng JPA, format multi-line `@Query` JPQL as a text block (`"""`), one clause per line, wrapped
in `// @formatter:off` / `// @formatter:on` so IDE/`mvn spotless` reformatting doesn't collapse or
re-indent it. Use this style for any non-trivial `@Query`, whether or not it has a `countQuery`:
...
@Query(
    // @formatter:off
        value = """
            SELECT p 
            FROM Entity p 
            JOIN FETCH p.relation r 
            WHERE r.someField = :someParam 
                AND p.otherField > :otherParam
            """
        )
        // @formatter:on
    List<Entity> findSomething(
            @Param("someParam") String someParam,
            @Param("otherParam") Integer otherParam);
```

When paginating (`Page<>` return type) and the query uses `JOIN FETCH`, add a matching
`countQuery` in the same text-block style — `Page<>` can't auto-derive a count from a `JOIN FETCH`
query, and `JOIN FETCH` isn't valid in a `COUNT(...)` projection, so the `countQuery` should use a
plain `JOIN` instead. Keep the `WHERE` clause identical between `value` and `countQuery` or the
pagination totals won't match the result page.
